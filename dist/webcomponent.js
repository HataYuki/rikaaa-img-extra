(function () {
  ...
  */
  ...
  var _css = ':host .wp {  background-color: blue; }.wp ::slotted(*) {  font-size: 100px;  color: red;  margin: 0; }';

  var _style = "<style>".concat(_css, "</style>");

  var _shadowdomHTML = "\n    ".concat(_style, "\n    <div class=\"wp\">\n        <slot class=\"slot_test\" name=\"text\"></slot>\n    </div>\n");

  var template = document.createElement('template');
  template.id = 'rikaaahoge';
  template.innerHTML = _shadowdomHTML;
  if (window.ShadyCSS) ShadyCSS.prepareTemplate(template, 'rikaaa-hoge');
  ...
  function (_HTMLElement) {
    ...
    createClass(rikaaahoge, [{
      ...
      value: function connectedCallback() {
        ...
        this._slot_test = this._wp.querySelector('.slot_test');
        this._text = filter$2(_context = this._slot_test.assignedNodes({
          flattern: true
        })).call(_context, function (n) {
          return n.nodeType === n.ELEMENT_NODE;
        });
        this.test = this._text[0].innerHTML;
        this.onCustomevent();
      }
    }, {
      key: "onCustomevent",
      value: function onCustomevent(f) {
        var oce = new CustomEvent('onCustomevent', {
          detail: {
            test: 'aaaaa'
          }
        });
        this.dispatchEvent(oce);
      }
    }, {
      key: "test",
      get: function get() {
        var attr = this.getAttribute('test');
        return attr;
      },
      set: function set(n) {
        this.setAttribute('test', n);
      }
    }]);
  ...
  ready(function () {
    customElements.define('rikaaa-hoge', rikaaahoge);
  });
